...
(function () {
  ...
  window.saveInstructor = async function saveInstructor() {
    ...
    } else if (window.database && window.firebaseModules) {
      window.instructors = data;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
      try {
        await window.firebaseModules.set(
          window.firebaseModules.ref(window.database, 'instructors'),
          data
        );
        if (typeof window.bumpLastSync === 'function') {
          await window.bumpLastSync();
        }
      } catch (e) {
        console.warn('[IAC] Firebase instrutores:', e);
      }
    } else {
      window.instructors = data;
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
