const STORAGE_KEY_COURSES = 'dashboard_courses';
    ...
    async function saveCalendarToFirebase() {
      ...
      try {
        ...
        await Promise.all([
          set(ref(window.database, 'calendarEvents'), calendarEvents),
          set(ref(window.database, 'calendarTasks'), calendarTasks),
        ]);
        await bumpLastSync();
      } catch (error) {
    ...
    function canEditInstructors() {
      return canEditCourses();
    }

    function requireInstructorEditAccess() {
      if (!canEditInstructors()) {
        if (!isLoggedIn) openLoginModal();
        else showToast('Faça login como administrador para gerenciar instrutores.', 'error');
        return false;
      }
      return true;
    }

    function showInstructorModalElement() {
    ...
    function hideInstructorModalElement() {
      const modal = document.getElementById('instructor-modal');
      if (!modal) return;
    ...
    if (!window.__IAC_INSTRUCTOR_BOOT__) {
    window.openInstructorModal = function openInstructorModal(id) {
      const modal = document.getElementById('instructor-modal');
      if (!modal) {
        if (typeof showToast === 'function') showToast('Formulário de instrutor não encontrado.', 'error');
        return;
      }
      const data = (window.instructors && typeof window.instructors === 'object') ? window.instructors : {};
      const inst = id && data[id] ? data[id] : { nome: '', telefone: '', cpf: '', tipoContrato: 'mei' };
      const title = document.getElementById('instructor-modal-title');
      const editId = document.getElementById('instructor-edit-id');
      const nomeEl = document.getElementById('instructor-nome');
      const telEl = document.getElementById('instructor-telefone');
      const cpfEl = document.getElementById('instructor-cpf');
      const tipoEl = document.getElementById('instructor-tipo');
      if (title) title.textContent = id ? 'Editar Instrutor' : 'Novo Instrutor';
      if (editId) editId.value = id || '';
      if (nomeEl) nomeEl.value = inst.nome || '';
      if (telEl) telEl.value = inst.telefone || '';
      if (cpfEl) cpfEl.value = inst.cpf || '';
      if (tipoEl) tipoEl.value = inst.tipoContrato || 'mei';
      modal.classList.remove('hidden');
      modal.style.display = 'flex';
      modal.style.alignItems = 'center';
      modal.style.justifyContent = 'center';
    ...
    window.ensureXLSX = async function ensureXLSX() {
      ...
      try {
        ...
        console.warn('Falha ao carregar XLSX:', error);
        return false;
      }
    };

    window.ensureJsPDF = async function ensureJsPDF() {
      if (window.jspdf) return true;
      try {
        await loadExternalScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js');
        return !!window.jspdf;
      } catch (error) {
        console.warn('Falha ao carregar jsPDF:', error);
        return false;
      }
    };
    ...
    function runWhenIdle(callback, timeoutMs = 1500) {
      if (typeof window.requestIdleCallback === 'function') {
        ...
        setTimeout(callback, Math.min(timeoutMs, 400));
      }
    }

    function getActiveTabName() {
      return activeTabCache || 'cronograma';
    }

    function scheduleCheckNotifications() {
      clearTimeout(notificationsTimer);
      notificationsTimer = setTimeout(() => checkNotifications(), 1200);
    }

    function renderVisibleTab(tabName) {
      const tab = tabName || getActiveTabName();
      activeTabCache = tab;
      ...
      if (tab === 'dashboard') {
        ...
        renderTarefas();
      } else if (tab === 'instructors' && typeof window.renderInstructorsList === 'function') {
        window.renderInstructorsList();
      }
    }
    ...
    function dedupeCoursesById(list) {
      const byId = new Map();
      normalizeAllCourses(list).forEach((course) => {
        if (course?.id) byId.set(course.id, course);
      });
      return Array.from(byId.values());
    }

    function coursesFromFirebaseValue(val) {
      if (!val) return [];
      let rawList = [];
      if (Array.isArray(val)) {
        rawList = val;
      } else if (typeof val === 'object') {
        rawList = Object.entries(val).map(([key, course]) => {
          if (course && typeof course === 'object') {
            return { ...course, id: course.id || key };
          }
          return course;
        });
      }
      return dedupeCoursesById(rawList);
    }

    function coursesToFirebaseMap(courseList) {
      const map = {};
      normalizeAllCourses(courseList).forEach((course) => {
        if (course?.id) map[course.id] = course;
      });
      return map;
    }

    function serializeCoursesForSync(courseList) {
      return JSON.stringify(normalizeAllCourses(courseList));
    }

    function scheduleRenderAll() {
      clearTimeout(renderAllTimer);
      renderAllTimer = setTimeout(() => renderVisibleTab(getActiveTabName()), 60);
    }
    ...
    function applyCoursesFromRemote(rawCourses, source = 'remote') {
      ...
      lastAppliedCoursesSignature = signature;
      courses = normalized;
    ...
    function applyResponsiblesFromRemote(newResponsibles, source = 'remote') {
      const data = newResponsibles || {};
      const signature = JSON.stringify(data);
      if (signature === lastAppliedResponsiblesSignature) return false;
    ...
    function applyChecklistsFromRemote(newChecklists, source = 'remote') {
      const data = newChecklists || {};
      const signature = JSON.stringify(data);
      if (signature === lastAppliedChecklistsSignature) return false;
      ...
      console.log(`🔄 Checklists sincronizados (${source})`);
      return true;
    }

    async function saveCoursePatch(courseId, patchFields) {
      const courseIndex = courses.findIndex(c => c.id === courseId);
      if (courseIndex === -1) return;

      Object.assign(courses[courseIndex], patchFields);
      courses[courseIndex] = normalizeCourseData(courses[courseIndex]);
      syncCourseToFiltered(courseId);
      localStorage.setItem(STORAGE_KEY_COURSES, JSON.stringify(courses));
      ...
      lastLocalSaveTimestamp = syncTimestamp;

      const updates = {
        [`courses/${courseId}`]: courses[courseIndex],
      ...
      try {
        ...
        if (update) {
          ...
          lastAppliedRemoteSync = syncTimestamp;
          localStorage.setItem('lastAppliedRemoteSync', syncTimestamp);
          return;
        }
      } catch (error) {
        console.warn('Erro no patch do curso, usando salvamento completo:', error);
      }

      await saveToStorage(true);
    }

    async function bumpLastSync() {
      if (!window.database || !window.firebaseModules) return null;
      const syncTimestamp = new Date().toISOString();
      lastLocalSaveTimestamp = syncTimestamp;
      lastAppliedRemoteSync = syncTimestamp;
      localStorage.setItem('lastAppliedRemoteSync', syncTimestamp);
      try {
        const { ref, set } = window.firebaseModules;
        await set(ref(window.database, 'lastSync'), syncTimestamp);
      } catch (error) {
        console.warn('Erro ao atualizar lastSync:', error);
      }
    ...
    async function saveToRealtimeDatabase() {
      ...
      try {
        ...
        const coursesToSave = coursesToFirebaseMap(courses);
        const syncTimestamp = new Date().toISOString();
        lastLocalSaveTimestamp = syncTimestamp;
        lastAppliedRemoteSync = syncTimestamp;
        lastAppliedCoursesSignature = serializeCoursesForSync(courses);
        ...
        await Promise.all([
          set(ref(db, 'courses'), coursesToSave),
          set(ref(db, 'responsibles'), lotResponsibles),
          set(ref(db, 'courseChecklists'), courseChecklists),
          set(ref(db, 'lastSync'), syncTimestamp),
        ]);
    ...
    async function saveToStorage(immediate = false) {
    ...
    async function pollRemoteSync(force = false) {
      if (!window.database || !window.firebaseModules || !isLoggedIn) return;
      ...
      try {
        const { ref, get } = window.firebaseModules;
        const snapshot = await get(ref(window.database, 'lastSync'));
        const remote = snapshot.val();
        if (!remote) return;
        if (!force && remote === lastAppliedRemoteSync) return;
        if (remote === lastLocalSaveTimestamp) {
        ...
        console.debug('Polling de sincronização:', error.code || error.message);
      }
    }

    function setupRealtimeListeners() {
      if (!window.database || !window.firebaseModules || !isLoggedIn) {
        console.warn('Listeners em tempo real não iniciados (Firebase ou login ausente)');
        return;
      }
      ...
      try {
        ...
        unsubscribeCalendarEvents = onValue(ref(db, 'calendarEvents'), (snapshot) => {
    ...
    async function applyUserRoleFromToken(user) {
      if (!user) {
        userRole = 'user';
        return;
      }
    ...
    function handleAuthenticatedSync(user) {
      ...
      refreshRemoteData()
        .then(() => {
          ...
          renderVisibleTab(getActiveTabName());
        })
        .catch((error) => {
    ...
    function initAuthAndSync() {
      if (authStateListenerStarted || !window.firebaseAuth || !window.firebaseModules) return false;
    ...
    window.bootFirebaseSync = function bootFirebaseSync() {
      if (!window.firebaseAuth || !window.firebaseModules) return;

      if (window.firebaseModules.goOnline && window.database) {
        window.firebaseModules.goOnline(window.database);
      }
      ...
      if (existingUser) {
        ...
        applyUserRoleFromToken(existingUser).then(() => {
          updateUIForLoginStatus();
          handleAuthenticatedSync(existingUser);
        });
      }
    };

    async function checkFirebaseAuth() {
      if (!window.firebaseAuth || !window.firebaseModules) return false;
      initAuthAndSync();
    ...
    function updateUIForLoginStatus() {
      ...
      if (isLoggedIn) {
        ...
        adminElements.forEach(el => {
          // Painéis de abas (main) são controlados por switchTab — não alterar hidden aqui
    ...
    async function waitForFirebaseReady(timeoutMs = 15000) {
      ...
      while (!window.firebaseAuth || !window.firebaseModules) {
        if (Date.now() - startedAt > timeoutMs) {
          console.error('❌ Firebase não carregou dentro do tempo limite');
          return false;
        }
    ...
    onAppReady(async () => {
      ...
      try {
        ...
        requestAnimationFrame(() => {
          ...
          if (!location.hash || location.hash === '#instructors' || location.hash.startsWith('#instructor/')) {
            location.hash = 'cronograma';
          }
          activeTabCache = 'cronograma';
          renderVisibleTab('cronograma');
        });
        runWhenIdle(() => checkAndUpdateAllCourseStatuses({ persist: false }), 2000);
      } catch (error) {
      ...
      setupVisibilitySync();
      setTimeout(() => {
        initFirebaseSync().catch(err => console.warn('Erro ao inicializar sincronização Firebase:', err));
      }, 200);
    });
    ...
    function isCourseEligibleForTarefas(course) {
      return !isCourseConcluido(course);
    }

    function getCoursesOutrasForTarefas() {
      return sortTarefasCourses(courses.filter(c =>
        isCourseEligibleForTarefas(c) &&
        !isCourseEmAndamento(c) &&
        !isCourseEmBreve(c)
      ));
    }

    function getAllCoursesForTarefas() {
      return sortTarefasCourses(courses.filter(isCourseEligibleForTarefas));
    }
    ...
    function renderTarefas() {
      courses.forEach((course) => updateCourseStatusAutomatically(course));
      ...
      const outrasList = document.getElementById('tarefas-outras-list');
      ...
      const outrasSection = document.getElementById('tarefas-outras-section');
      ...
      const outrasAll = getCoursesOutrasForTarefas();
      const totalAll = getAllCoursesForTarefas().length;

      const emAndamento = filterTarefasBySearch(emAndamentoAll);
      const emBreve = filterTarefasBySearch(emBreveAll);
      const outras = filterTarefasBySearch(outrasAll);
      const totalFiltered = emBreve.length + emAndamento.length + outras.length;
      const hasSearch = !!document.getElementById('tarefas-search')?.value.trim();

      let totalItems = 0;
      let totalChecked = 0;
      getAllCoursesForTarefas().forEach(course => {
      ...
      if (statsEl) {
        statsEl.innerHTML = `
          <div class="card-elegant rounded-[16px] p-4">
            <p class="text-xs text-muted-foreground mb-1">Total de turmas</p>
            <p class="text-2xl font-bold text-foreground">${hasSearch ? totalFiltered : totalAll}</p>
          </div>
          <div class="card-elegant rounded-[16px] p-4">
      ...
      if (totalAll === 0) {
        ...
        outrasSection?.classList.add('hidden');
        return;
      }
      ...
      if (outrasList && outrasSection) {
        if (outras.length > 0) {
          outrasSection.classList.remove('hidden');
          outrasList.innerHTML = outras.map(renderTarefasShelfCard).join('');
        } else {
          outrasSection.classList.add('hidden');
          outrasList.innerHTML = '';
        }
      }
    }
    ...
    function renderAll() {
      ...
      const instructorsContent = document.getElementById('content-instructors');
      if (instructorsContent && !instructorsContent.classList.contains('hidden') && typeof window.renderInstructorsList === 'function') {
        window.renderInstructorsList();
      }
    ...
    function navigateToCourse(courseId) {
      ...
      if (typeof openCoursePage === 'function') {
      ...
      container.innerHTML = `
        ...
        ${upcomingEvents.length > 0 ? `
          <div class="card-elegant rounded-[20px] p-5 cursor-pointer hover:scale-[1.02] transition-all" onclick="filterByUpcoming()">
            ...
            <div class="mt-3 text-sm text-[#FF9500] font-medium">Ver cronograma →</div>
          </div>
        ` : ''}
        ...
        return `
          <div class="card-elegant rounded-[20px] overflow-hidden relative mb-6">
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  ${lotCourses.map(course => `
                    <tr class="hover:bg-[rgba(0,0,0,0.02)] transition-colors cursor-pointer border-b border-[rgba(0,0,0,0.05)]" onclick="openCoursePage('${course.id}')">
            ...
            <div class="md:hidden p-4 space-y-3">
              ${lotCourses.map(course => `
                <div class="bg-[rgba(255,255,255,0.5)] border border-[rgba(0,0,0,0.1)] rounded-[16px] p-4 space-y-3 cursor-pointer hover:bg-[rgba(255,255,255,0.7)] transition-colors" onclick="openCoursePage('${course.id}')">
    ...
    async function exportSpreadsheet() {
      ...
      const xlsxReady = await window.ensureXLSX();
      if (!xlsxReady) {
    ...
    async function downloadLotPDF(lotNumber) {
      showToast('Gerando PDF...', 'info');
      const pdfReady = await window.ensureJsPDF();
      if (!pdfReady) {
    ...
    async function downloadAllLotsPDF() {
      if (!requireEditAccess()) return;
    ...
    async function downloadCronogramaMesAtualPDF(selectedCourses = null) {
      if (!requireEditAccess()) return;
    ...
    function renderCronogramaCardHtml(course, options = {}) {
      ...
      return `
          <div class="card-elegant rounded-[20px] p-5 cursor-pointer hover:scale-[1.02] transition-all duration-200 ${highlight ? 'ring-2 ring-[#007AFF]/30' : ''}" onclick="openCoursePage('${course.id}')">
            <div class="space-y-3">
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-1">
              ...
              <div class="space-y-2 text-sm">
                ...
                ` : ''}
                ${course.observacoes && String(course.observacoes).trim() ? `
                <div class="cronograma-card-obs mt-1 p-2.5 rounded-xl bg-[rgba(255,149,0,0.08)] border border-[rgba(255,149,0,0.15)]">
                  <p class="text-[10px] font-semibold text-[#FF9500] mb-0.5">Observações</p>
                  <p class="text-xs text-[#1d1d1f] dark:text-[#f5f5f7] line-clamp-3 whitespace-pre-wrap">${escapeHtml(String(course.observacoes).trim())}</p>
                </div>
                ` : ''}
              </div>
            ...
            <div class="card-elegant rounded-[20px] p-6">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ${sortedGroupCourses.map(course => {
                  const statusClass = getStatusClass(course.status);
                  return `
                    <div class="card-secondary rounded-[16px] p-4 cursor-pointer hover:scale-[1.02] transition-all" onclick="openCoursePage('${course.id}')">
                      <div class="space-y-2">
                        <div class="flex items-start justify-between">
              ...
              ${group.courses.map((course, index) => {
                ...
                return `
                  <div class="relative mb-6">
                    ...
                    <!-- Card do curso -->
                    <div class="card-elegant rounded-[16px] p-5 ml-4 cursor-pointer hover:scale-[1.01] transition-all" onclick="openCoursePage('${course.id}')">
                      <div class="flex items-start justify-between gap-4">
                        <div class="flex-1">
    ...
    function openDriveLink(courseId, type) {
      ...
      if (link) {
        ...
        openCronogramaCourseModal(courseId);
        setTimeout(() => {
          const inputId = type === 'album' ? 'cronograma-edit-album' : 'cronograma-edit-instrumentais';
    ...
    function renderCalendar() {
      const calendarContainer = document.getElementById('calendar-tab-container');
          ...
          <div class="h-[110px] border-2 ${isToday ? 'border-[#007AFF] bg-gradient-to-br from-[rgba(0,122,255,0.15)] to-[rgba(0,122,255,0.05)] shadow-md' : hasContent ? 'border-border/40 bg-card/50' : 'border-border/20 bg-secondary/20'} rounded-[12px] p-2 overflow-hidden ${hasContent ? 'cursor-pointer' : ''} hover:border-[#007AFF]/50 hover:shadow-md transition-all duration-200" 
            ...
            ${visibleCourses.map(c => {
              ...
              <div class="group text-[10px] bg-[rgba(0,122,255,0.25)] dark:bg-[rgba(0,122,255,0.4)] text-[#007AFF] dark:text-[#5AC8FA] rounded-[6px] px-1.5 py-1 mb-0.5 cursor-pointer hover:bg-[rgba(0,122,255,0.35)] dark:hover:bg-[rgba(0,122,255,0.5)] transition-all" 
                   onclick="event.stopPropagation(); openCoursePage('${c.id}')"
    ...
    function handleDayClick(event, dateStr) {
      ...
      if (target.tagName === 'BUTTON' || 
          ...
          target.closest('[onclick*="openCoursePage"]') !== null ||
          ...
          <div class="mb-6">
            ...
            <div class="space-y-3">
              ${dayCourses.map(c => {
                const isStart = c.dataInicio && c.dataInicio.split('T')[0] === dateStr;
                return `
                  <div class="p-4 border border-[rgba(0,122,255,0.3)] bg-[rgba(0,122,255,0.1)] dark:bg-[rgba(0,122,255,0.2)] rounded-lg cursor-pointer hover:bg-[rgba(0,122,255,0.15)] dark:hover:bg-[rgba(0,122,255,0.3)] transition-colors" onclick="openCoursePage('${c.id}')">
                    <div class="flex items-start justify-between">
                      <div class="flex-1">
    ...
    async function toggleTurmaFormada(courseId) {
      if (!requireEditAccess()) return;
      ...
      const newValue = !isTurmaFechada(courses[courseIndex]);
      courses[courseIndex].turmaFormada = newValue;
      
      logAction(newValue ? 'Turma Formada Marcada' : 'Turma Formada Desmarcada', {
        courseId: courses[courseIndex].idCurso,
        tipologia: courses[courseIndex].tipologia
      });

      try {
        await saveCoursePatch(courseId, { turmaFormada: newValue });
        renderAll();
        const courseModal = document.getElementById('course-modal');
        if (typeof renderCoursePage === 'function' && courseModal && !courseModal.classList.contains('hidden')) {
          renderCoursePage(courseId);
        }
        showToast(newValue ? 'Turma formada marcada!' : 'Turma formada desmarcada!', 'success');
      } catch (error) {
        console.error('Erro ao salvar turma formada:', error);
        showToast('Erro ao sincronizar turma formada.', 'error');
      }
    }
    ...
    function showToast(message, type = 'info', duration = 3000, showUndo = false, undoAction = null) {
      ...
      const undoButton = showUndo ? `
        <button onclick="undoLastAction(); this.closest('[data-toast]').remove()" class="ml-2 px-3 py-1 bg-white/20 hover:bg-white/30 rounded text-sm font-medium transition-colors whitespace-nowrap">
          Desfazer
        </button>
      ` : '';
      ...
      try {
        switch(lastAction.action) {
          ...
          case 'deleteInstructor':
            if (lastAction.data.instructor && lastAction.data.allInstructors) {
              const restoredInstructors = { ...lastAction.data.allInstructors };
              restoredInstructors[lastAction.data.instructor.id] = lastAction.data.instructor;
              if (typeof window.saveInstructorsToFirebase === 'function') {
                window.saveInstructorsToFirebase(restoredInstructors);
              } else {
                window.instructors = restoredInstructors;
                localStorage.setItem('instructors_data', JSON.stringify(restoredInstructors));
              }
              if (typeof window.refreshInstructorsUI === 'function') window.refreshInstructorsUI();
    ...
    window.canEditCourses = canEditCourses;
    window.requireEditAccess = requireEditAccess;
    window.canEditInstructors = canEditInstructors;
    window.requireInstructorEditAccess = requireInstructorEditAccess;
    window.openLoginModal = openLoginModal;
    window.showToast = showToast;
    window.switchTab = switchTab;
    window.bumpLastSync = bumpLastSync;
    window.addToUndoStack = addToUndoStack;
    ...
    function bindAddInstructorButton() {
      ...
      window._instructorBtnBound = true;
      document.addEventListener('click', function (e) {
        const btn = e.target.closest('#btn-add-instructor');
        if (!btn) return;
        e.preventDefault();
        e.stopPropagation();
    ...
    window.bindAddInstructorButton = bindAddInstructorButton;
    if (!window.__IAC_INSTRUCTOR_BOOT__) {
      bindAddInstructorButton();
    }
